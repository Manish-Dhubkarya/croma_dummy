...
export default function SmallScreenCartComponent(props) {
  ...
  const options = {
    ...
    handler: async function (response) {
      ...
      if (result.status) { alert('Order submitted') }
      else {
    ...
    <div style={{ width: '82%', marginTop: '31%' }}>
      ...
      <div style={{ marginTop: '5%', padding: 5, border: '1px solid #fff', width: '100%', borderRadius: 8, background: '#fff' }}>
        ...
        {props.title == "Checkout" ?
          <div onClick={handleClick} style={{ cursor:"pointer", border: '1px solid #12DAA8', width: '100%', borderRadius: 6, padding: 2, marginTop: '5%', display: 'flex', justifyContent: 'center', background: '#12DAA8', fontSize: 14 }}>

            <div style={{ padding: 6, cursor: 'pointer' }}><b>{props.title} </b></div>
          </div>
          :
          <div onClick={handlePayment} style={{ border: '1px solid #12DAA8', cursor:"pointer", width: '100%', borderRadius: 6, padding: 2, marginTop: '5%', display: 'flex', justifyContent: 'center', background: '#12DAA8', fontSize: 14 }}>
            <div style={{padding:6}}><b>{props.title} </b></div>
          </div>}
      </div>
    </div>
